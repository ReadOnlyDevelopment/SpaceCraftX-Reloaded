...
public class ModelOxygenTankItem extends ModelTransformWrapper {

	public ModelOxygenTankItem(IBakedModel modelToWrap) {
		super(modelToWrap);
	}
	...
	protected Matrix4f getTransformForPerspective(ItemCameraTransforms.TransformType cameraTransformType) {
		if (cameraTransformType == ItemCameraTransforms.TransformType.GUI) {
			...
			ret.mul(mul);
			mul.setIdentity();
			mul.setTranslation(new Vector3f(0.0F, -0.25F, -0.6F));
			ret.mul(mul);
			mul.setIdentity();
			mul.rotY(1.5707964F);
			ret.mul(mul);
		...
		if (cameraTransformType == ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND || cameraTransformType == ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND) {
			Vector3f trans = new Vector3f(0.5F, -3.9F, -2.6F);
			Matrix4f ret = new Matrix4f();
			ret.setIdentity();
			Matrix4f mul = new Matrix4f();
			mul.setIdentity();
		...
		if (cameraTransformType == ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND || cameraTransformType == ItemCameraTransforms.TransformType.THIRD_PERSON_LEFT_HAND) {
			Vector3f trans = new Vector3f(-0.6F, -3.8F, 1.1F);
			Matrix4f ret = new Matrix4f();
			ret.setIdentity();
			Matrix4f mul = new Matrix4f();
			mul.setIdentity();
		...
		if (cameraTransformType == ItemCameraTransforms.TransformType.FIXED) {
			...
			ret.mul(mul);
			mul.setIdentity();
			mul.setTranslation(new Vector3f(0.5F, -2.8F, 0.5F));
			ret.mul(mul);
			return ret;
		}
		return null;
