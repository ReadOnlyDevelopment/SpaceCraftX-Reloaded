...
public class Falcon9RocketSeparatedItem extends Item implements IHoldableItem, ISortableItem {
	public Falcon9RocketSeparatedItem(String assetName) {
		setMaxDamage(0);
		setHasSubtypes(true);
		setMaxStackSize(1);
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (padFound) {
			if (!placeRocketOnPad(stack, worldIn, pos, tile, centerX, centerY, centerZ))
	...
	public static boolean placeRocketOnPad(ItemStack stack, World worldIn, BlockPos pos, TileEntity tile, float centerX, float centerY, float centerZ) {
		...
		EntityFalcon9Rocket spaceship = new EntityFalcon9Rocket(worldIn, centerX, centerY, centerZ, IRocketType.EnumRocketType.values()[stack.getItemDamage()], pos.getY());
		...
		spaceship.separateStages(true);
		spaceship.setLaunchPhase(EntitySpaceshipBase.EnumLaunchPhase.UNIGNITED);
		spaceship.setF9LaunchPhase(EntityFalcon9Rocket.EnumF9LaunchPhase.LANDED);
		spaceship.deployLegs(true);
		spaceship.setLegsAngle(117.0F);
		spaceship.deployGridsFins(true);
		spaceship.setGridsFinsAngle(90.0F);
	...
	public void addInformation(ItemStack par1ItemStack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		tooltip.add(EnumColor.RED + SpaceXUtils.translate("gui.message.falcon9.used"));
		if (type.getPreFueled())
			tooltip.add(EnumColor.RED + "§o" + GCCoreUtil.translate("gui.creative_only.desc"));
		if (par1ItemStack.hasTagCompound() && par1ItemStack.getTagCompound().hasKey("RocketFuel")) {
			EntityFalcon9Rocket rocket = new EntityFalcon9Rocket((World) FMLClientHandler.instance().getWorldClient(), 0.0D, 0.0D, 0.0D, IRocketType.EnumRocketType.values()[par1ItemStack.getItemDamage()], 0.0D);
