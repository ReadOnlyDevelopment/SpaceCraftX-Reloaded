...
public class F9SecondStageRocketSeparatedItem extends Item implements IHoldableItem, ISortableItem {
	...
	public static boolean placeRocketOnPad(ItemStack stack, World worldIn, TileEntity tile, float centerX, float centerY, float centerZ) {
		if (tile instanceof TileEntityLandingPad) {
			TileEntityLandingPad dock = (TileEntityLandingPad) tile;
			if (dock.getDockedEntity() != null)
				return false;
		} else
			return false;
		EntityF9SecondStage spaceship = new EntityF9SecondStage(worldIn, centerX, centerY, centerZ, IRocketType.EnumRocketType.values()[stack.getItemDamage()]);
	...
	public F9SecondStageRocketSeparatedItem(String assetName) {
		setMaxDamage(0);
		setHasSubtypes(true);
		setMaxStackSize(1);
	...
	public void addInformation(ItemStack par1ItemStack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		if (par1ItemStack.hasTagCompound() && par1ItemStack.getTagCompound().hasKey("RocketFuel")) {
			EntityFalcon9Rocket rocket = new EntityFalcon9Rocket(FMLClientHandler.instance().getWorldClient(), 0.0D, 0.0D, 0.0D, IRocketType.EnumRocketType.values()[par1ItemStack.getItemDamage()], 0.0D);
